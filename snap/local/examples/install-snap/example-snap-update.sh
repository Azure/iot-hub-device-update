...
log() {
    update_timestamp
    [ -z $log_file ] && echo -e "[$_timestamp]" "$@" >&1 || echo "[$_timestamp]" "$@" >> "$log_file"
}

# Write to output file
output() {
    update_timestamp
    [ -z $output_file ] && echo "[$_timestamp]" "$@" >&1 || echo "[$_timestamp]" "$@" >> "$output_file"
}
...
function is_installed() {
    ...
    response=$(curl --unix-socket /run/snapd.socket "http://localhost/v2/snaps/$snap_name?select=version,channel")
...
InstallUpdate() {
    ...
    if [[ $is_installed_ret -ne 0 ]]; then
        ...
        if [ -z "$status_code" ]; then
            ...
            resultDetails="Install request error ('$installed_criteria', params: $snapd_api_data, status: $status_code, status text: $status_text)"
            resultCode=0
            # ADUC_ERC_SCRIPT_HANDLER_INSTALL_INSTALLITEM_BAD_DATA, ERC Value: 810549762 (0x30500202)
            extendedResultCode=$((0x30500202))
            error "$resultDetails"
...
#
# Restore the device to the previous state before the update has been applied.
#
DoRestore() {
    # TODO - Implement restore logic.
    # 1101 - ADUC_Result_Restore_Success_Unsupported
    log_info "Restore action is not supported. Returning 1101."

    resultCode=1101
    extendedResultCode=0
    resultDetails="Restore action is not supported."
    ret_val=0
    prepare_and_save_aduc_result "$resultCode" "$extendedResultCode" "$resultDetails"
    $ret $ret_val
}
...
#
# This function is called before the update is applied.
# It is used to backup the current device state before the update is applied.
# This is useful in case the update fails and the device needs to be restored to its previous state.
#
DoBackup() {
    # TOOD - Implement backup logic.
    # 1001 - ADUC_Result_Backup_Success_Unsupported
    log_info "Backup action is not supported. Returning 1001."

    resultCode=1001
    extendedResultCode=0
    resultDetails="Backup action is not supported."
    ret_val=0
    prepare_and_save_aduc_result "$resultCode" "$extendedResultCode" "$resultDetails"
    $ret $ret_val
}
...
if [ -n "$do_backup_action" ]; then
    DoBackup
    exit $ret_val
fi

if [ -n "$do_restore_action" ]; then
    DoRestore
    exit $ret_val
fi
