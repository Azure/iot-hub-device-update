...
| InstalledCriteria | string | String interpreted by the agent to determine if the update completed successfully:  <ul><li>Specify **value** of "/etc/adu-version" set in [CMAKE](https://github.com/Azure/iot-hub-device-update/blob/367b880baf43294f8b127612f7687e1247d50c79/CMakeLists.txt) .</li><li>Specify `{name}-{version}` for update type `microsoft/apt:1`, of which name and version are obtained from the APT file.</li><li>Specify a custom string if developing a custom agent.</li></ul> | Maximum of 64 characters |
| Compatibility | Array of [`CompatibilityInfo` objects](https://docs.microsoft.com/en-us/azure/iot-hub-device-update/import-schema#compatibilityinfo-object) | Compatibility information of device compatible with this update. | Maximum of 10 items |
