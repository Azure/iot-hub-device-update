...
In the latest Public Preview refresh release, the Device Update Agent will support multiple Update Types at the same time for the Public Preview refresh agent. The Public Preview refresh agent will also support a Content Handler extension capability where device builders can also define a custom Update Type and implement an associated custom Content Handler as needed for their additional device updates and deployment scenarios.
...
Below, you can find example of useful helper funcitons used in most handlers provided in this project.  
...
|workflow_get_update_files_count|Get total update playload files count|
...
In this case, if the Update has been authored and imported correctly, the DU Agent workflow will include a 'Selected Components' data in the ADUC_WorkfloHandle object thas pass to the handler's function.  

When implementing handler that handles an update intented for connected-components, the following helper fuctions can be used to get selected components properties:
...
Provide Content Handler Extensions
...
│       ├── apt_parser_ut.cpp
│       ├── main.cpp
│       └── sample_apt_manifest.json
├── inc
│   └── aduc
│       └── content_handler_factory.hpp
├── script_handler
│   ├── CMakeLists.txt
│   ├── inc
│   │   └── aduc
...
│   ├── inc
│   │   └── aduc
│   │       └── simulator_handler.hpp
│   ├── src
│   │   └── simulator_handler.cpp
│   └── tests
│       ├── CMakeLists.txt
│       ├── main.cpp
│       └── simulator_handler_unit_tests.cpp
├── src
│   └── content_handler_factory.cpp
├── steps_handler
│   ├── CMakeLists.txt
│   ├── inc
│   │   └── aduc
