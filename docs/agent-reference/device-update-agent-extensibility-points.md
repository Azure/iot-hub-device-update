...
The DeviceUpdate agent installed via the Debian package will include the [Microsoft Delta Download Handler](../../src/extensions/download_handlers/plugin_examples/microsoft_delta_download_handler/handler/plugin/src/microsoft_delta_download_handler_plugin.EXPORTS.c) plugin.  This download handler implements `Delta Updates` using the [Diff API](https://github.com/Azure/iot-hub-device-update-delta) to apply the diff to recreate the update payload that was generated by [Diff Generation](https://github.com/Azure/iot-hub-device-update-delta) from a source swupdate .swu file and the target .swu update payload.
