...
while [[ $1 != "" ]]; do
    ...
    --output-file)
        shift
        if [[ -z $1 || $1 == -* ]]; then
            error "--output-file parameter is mandatory."
            $ret 1
        fi
        output_file="$1"
...
#
# Example implementation of 'DownloadUpdateFiles' function, for this update scenario.
#
# This fuction is no-op since no additional files are required for this update.
#
DownloadUpdateFiles() {
    log_info "DownloadUpdateFiles called"
