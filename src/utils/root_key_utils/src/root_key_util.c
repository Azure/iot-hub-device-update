...
#define NULL_OR_STR(STR) IsNullOrEmpty((STR)) ? "NULL" : (STR)
...
ADUC_Result RootKeyUtility_GetAlgAndPublicKeyHashSigningKeyFromSigningKeyPayload(const char* decodedSigningKeyPayload, char* outHashAlgStr, CONSTBUFFER* outHashPublicKey)
{
    ...
    char* payload_n_modulus = NULL;
    char* payload_exponent = NULL;

    JSON_Value* rootJsonValue = NULL;
    JSON_Object* rootJsonObj = NULL;
    const char* jsonStrVal = NULL;
    unsigned char* modulus_buf = NULL;

    if (IsNullOrEmpty(signingKeyPayload) || IsNullOrEmpty(outHashAlgStr) || outHashPublicKey == NULL)
    {
    ...
    Log_Debug("alg: '%s'", NULL_OR_STR(jsonStrVal));
    ...
    Log_Debug("kid: '%s'", NULL_OR_STR(jsonStrVal));

    // n, or RSA modulus
    jsonStrVal = json_object_get_string(rootjsonObj, "n");
    if (IsNullOrEmpty(jsonStrVal))
    {
        result.ExtendedResultCode = ADUC_ERC_UTILITIES_ROOTKEYUTIL_SIGNING_KEY_INVALID_N;
        goto done;
    }

    if (Base64URLDecode(payload_n_modulus, unsigned char** decoded_buffer) == 0)
    {
    }

    // e, or RSA exponent
    jsonStrVal = json_object_get_string(rootjsonObj, "e");
    ...
    if (mallocAndStrcpy_s(&payload_exponent, jsonStrVal) != 0)
    {
    ...
    *outHashPublicKey = hashPubKeyBuf;
    ...
    free(payload_n_modulus);
    free(payload_exponent);
...
ADUC_Result RootKeyUtility_IsSigningKeyDisallowed(const char* payload_alg, const CONSTBUFFER payload_hashPublicKeySigningKey, bool* is_signing_key_disallowed);
{
    ...
    result = ADUC_RootKeyPackageUtils_IsSigningKeyInDisabledList(localStore, payload_alg, payload_hashPublicKeySigningKey, &isDisallowed);
    if (IsAducResultCodeFailure(result.ResultCode))
    {
