...
ADUC_Result CryptoUtils_EncryptBuffer(const char* alg, const KeyData* eKey, const char* iv,const unsigned char* plainTextBuff, const size_t plainTextBuffSize, char* destBuff, size_t* destBuffSize)
{
    ADUC_Result result = { .ResultCode = ADUC_GeneralResult_Failure , .ExtendedResultCode = 0};

    unsigned char* encryptedBuff = NULL;
    if (alg == NULL || eKey == NULL || plainTextBuff == NULL || destBuff == NULL)
    {
        goto done;
    }
...
bool CryptoUtils_CalculateIV()
{

}

ADUC_Result CryptoUtils_DecryptBuffer(const char* alg, const KeyData* dKey,  const unsigned char* encryptedBuff, const size_t encryptedBuffSize, char** destBuff, size_t* destBuffSize)
{
    ADUC_Result result = { .ResultCode = ADUC_GeneralResult_Failure , .ExtendedResultCode = 0};

    unsigned char* decryptedBuffer = NULL;

    if (dKey == NULL || dKey->keyData == NULL || encryptedBuff == NULL || destBuff == NULL)
    {
        result.ExtendedResultCode = ADUC_ERC_NOMEM;
        goto done;
    }
    ...
    const char* iv = CryptoUtils_CalculateIV();
