...
extern "C"
{
    ...
    static void Mock_WorkCompletionCallback_InstallFailed(const void* workCompletionToken, ADUC_Result result, bool isAsync)
    {
        ...
        switch (s_mockWorkCompletionCallbackCallCount)
        {
            ...
            case 4:
                ...
                //
                REQUIRE(isAsync);
                REQUIRE(ADUC_WorkflowData_GetLastReportedState(workflowData) == ADUCITF_State_RestoreStarted);
                REQUIRE(ADUC_WorkflowData_GetCurrentAction(workflowData) == ADUCITF_UpdateAction_ProcessDeployment);
                REQUIRE(workflow_get_current_workflowstep(workflowData->WorkflowHandle) == ADUCITF_WorkflowStep_Restore);
    ...
    static void Mock_WorkCompletionCallback_ApplyFailed(const void* workCompletionToken, ADUC_Result result, bool isAsync)
    {
        CHECK(workCompletionToken != nullptr);
        CHECK(result.ExtendedResultCode == 0);
...
static ADUC_ResultCode s_apply_result_code = ADUC_Result_Failure;
...
static void Reset_Mocks_State_Install_Failed()
{
    ...
    s_install_result_code = ADUC_Result_Failure;
    s_apply_result_code = ADUC_Result_Failure;
...
static void Reset_Mocks_State_Apply_Failed()
{
    s_mockWorkCompletionCallbackCallCount = 0;
    s_workflowComplete = false;
    s_expectedWorkflowIdWhenIdle = "";
...
// This test exercises the install failure path for the entire agent-orchestrated workflow
// via HandlePropertyUpdate, but with mocked
// ADUC_Workflow_WorkCompletionCallback and mocked ContentHandler layer.
// It does not depend on simulator platform / simulator content handler and
// exercises the platform layer as well, including the async worker threads for
