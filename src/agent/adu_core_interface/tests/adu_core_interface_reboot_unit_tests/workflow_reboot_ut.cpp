...
public:
    ...
    ADUC_Result Backup(const ADUC_WorkflowData* workflowData) override {
        ...
        ADUC_Result result = { s_backup_result_code, 0 };
        return result;
    }
    ...
    ADUC_Result Restore(const ADUC_WorkflowData* workflowData) override {
        ...
        ADUC_Result result = { s_restore_result_code, 0 };
        return result;
    }
...
public:
    WorkflowRebootManagedWorkflowData()
    {
        ...
        // Disable update manifest validation
...
TEST_CASE_METHOD(TestCaseFixture, "Process Workflow Apply - Reboot Success")
{
    ...
    std::string expectedClientReportingString = aduc::FileTestUtils_slurpFile(std::string{ ADUC_TEST_DATA_FOLDER } + "/workflow_reboot/expectedClientReportingStringAfterReboot.json");
