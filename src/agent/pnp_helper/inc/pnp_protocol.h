...
//
#define PNP_STATUS_SUCCESS 200 //!< PNP_STATUS_SUCCESS
#define PNP_STATUS_BAD_FORMAT 400 //!< PNP_STATUS_BAD_FORMAT
#define PNP_STATUS_NOT_FOUND 404 //!< PNP_STATUS_NOT_FOUND
#define PNP_STATUS_INTERNAL_ERROR 500 //!< PNP_STATUS_INTERNAL_ERROR

//
// The PnP convention defines the maximum length of a component
//
#define PNP_MAXIMUM_COMPONENT_LENGTH 64 //!< PNP_MAXIMUM_COMPONENT_LENGTH
...
/**
* @brief PnP_CreateReportedProperty returns JSON to report a property's value from the device.  This does NOT contain any metadata such as
* a result code or version.  It is used when sending properties that are NOT marked as <"writable": true> in the DTDL defining
* the given property. The application itself needs to send this to Device Twin, using a function such as ClientHandle_SendReportedState.
* @param componentName name of the component to report
* @param propertyName the property to update with @p propertyValue
* @param propertyValue the value to set @p propertyName too
* @returns the STRING_HANDLE representation of the reported property
*/
STRING_HANDLE
PnP_CreateReportedProperty(const char* componentName, const char* propertyName, const char* propertyValue);

/**
* @brief PnP_CreateReportedProperty returns JSON to report a property's value from the device.  This contains metadata such as
...
* @param ackVersion the version to be acknowledged
*/
...
/** 
* @brief PnP_ParseCommandName is invoked by the application when an incoming device method arrives.  This function
...
* @param deviceMethodName the method name for the device
* @param componentName the name of the component to have a command parsed for
* @param componentNameSize the size of the component name 
* @param pnpCommandName the value that will be loaded with the pnp command's name
*/
...
/**
* @brief PnP_CreateTelemetryMessageHandle creates an IOTHUB_MESSAGE_HANDLE that contains tho contents of the telemetryData.
...
* @param componentName the component to create the message handle for
* @param telemetryData the telemetry data to be assigned with message handle
* @returns a value of IOTHUB_MESSAGE_HANDLE
*/
IOTHUB_MESSAGE_HANDLE PnP_CreateTelemetryMessageHandle(const char* componentName, const char* telemetryData);

/**
* @brief PnP_ProcessTwinData is invoked by the application when a device twin arrives to its device twin processing callback.
...
* @param updateState the state of the update to be processed
* @param payload the payload to be delivered to the device
* @param size the size of the payload
* @param componentsInModel the number of components in the model
* @param pnpPropertyCallback the callback functions for the DoWorks of each component
* @param userContectCallback the context that will be used to re-enter the common execution
...
/**
* @brief PnP_CopyTwinPayloadToString takes the payload data, which arrives as a potentially non-NULL terminated string from the IoTHub SDK, and creates
...
* @param payload the payload to copy to a string
* @param size the size of payload
* @returns the payload in string format
*/
