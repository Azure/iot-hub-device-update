...
//
static IOTHUB_DEVICE_CLIENT_LL_HANDLE
AllocateDeviceClientHandle(const PNP_DEVICE_CONFIGURATION* pnpDeviceConfiguration)
{
    IOTHUB_DEVICE_CLIENT_LL_HANDLE deviceHandle = NULL;

    if (pnpDeviceConfiguration->securityType == PNP_CONNECTION_SECURITY_TYPE_CONNECTION_STRING)
    {
        if ((deviceHandle = IoTHubDeviceClient_LL_CreateFromConnectionString(
                 pnpDeviceConfiguration->u.connectionString, MQTT_Protocol))
            == NULL)
        {
...
IOTHUB_DEVICE_CLIENT_LL_HANDLE PnP_CreateDeviceClientLLHandle(const PNP_DEVICE_CONFIGURATION* pnpDeviceConfiguration)
{
    ...
    // Sets verbosity level
        ...
        (iothubResult =
             IoTHubDeviceClient_LL_SetOption(deviceHandle, OPTION_LOG_TRACE, &pnpDeviceConfiguration->enableTracing))
        != IOTHUB_CLIENT_OK)
    {
    ...
    // handle is created, but will implicitly connect to subscribe for device method and device twin callbacks below.
        ...
        (iothubResult =
             IoTHubDeviceClient_LL_SetOption(deviceHandle, OPTION_MODEL_ID, pnpDeviceConfiguration->modelId))
        != IOTHUB_CLIENT_OK)
    {
    ...
    // Optionally, set the callback function that processes incoming device methods, which is the channel PnP Commands are transferred over
        ...
        (pnpDeviceConfiguration->deviceMethodCallback != NULL)
        && (iothubResult = IoTHubDeviceClient_LL_SetDeviceMethodCallback(
                deviceHandle, pnpDeviceConfiguration->deviceMethodCallback, NULL))
            != IOTHUB_CLIENT_OK)
    {
    ...
    // transferred over.  This will also automatically retrieve the full twin for the application on startup.
        ...
        (pnpDeviceConfiguration->deviceTwinCallback != NULL)
        && (iothubResult = IoTHubDeviceClient_LL_SetDeviceTwinCallback(
                deviceHandle, pnpDeviceConfiguration->deviceTwinCallback, (void*)deviceHandle))
            != IOTHUB_CLIENT_OK)
    {
    ...
    // Enabling auto url encode will have the underlying SDK perform URL encoding operations automatically.
        ...
        (iothubResult =
             IoTHubDeviceClient_LL_SetOption(deviceHandle, OPTION_AUTO_URL_ENCODE_DECODE, &urlAutoEncodeDecode))
        != IOTHUB_CLIENT_OK)
    {
    ...
    // Setting the Trusted Certificate.  This is only necessary on systems without built in certificate stores.
        ...
        (iothubResult = IoTHubDeviceClient_LL_SetOption(deviceHandle, OPTION_TRUSTED_CERT, certificates))
        != IOTHUB_CLIENT_OK)
    {
