...
static const char g_propertyWithResponseSchemaWithoutComponent[] =
...
PnP_CreateReportedProperty(const char* componentName, const char* propertyName, const char* propertyValue)
{
    ...
    {
        jsonToSend =
            STRING_construct_sprintf(g_propertyWithoutResponseSchemaWithoutComponent, propertyName, propertyValue);
    }
    else
    {
        jsonToSend = STRING_construct_sprintf(
            g_propertyWithoutResponseSchemaWithComponent, componentName, propertyName, propertyValue);
    }
    ...
    int ackVersion)
{
    ...
    {
        jsonToSend = STRING_construct_sprintf(
            g_propertyWithResponseSchemaWithoutComponent,
            propertyName,
            propertyValue,
            result,
            description,
    ...
    {
        jsonToSend = STRING_construct_sprintf(
            g_propertyWithResponseSchemaWithComponent,
            componentName,
            propertyName,
            propertyValue,
            result,
...
//
static bool
IsJsonObjectAComponentInModel(const char* objectName, const char** componentsInModel, size_t numComponentsInModel)
{
    bool result = false;
...
//
static bool VisitDesiredObject(
    ...
    void* userContextCallback)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < numChildren; i++)
        {
            ...
            if ((json_type(value) == JSONObject)
                && IsJsonObjectAComponentInModel(name, componentsInModel, numComponentsInModel))
            {
    ...
    void* userContextCallback)
{
    ...
    else
    {
        ...
        result = VisitDesiredObject(
            desiredObject, componentsInModel, numComponentsInModel, pnpPropertyCallback, userContextCallback);
    }
