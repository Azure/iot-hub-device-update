...
TEST_CASE("apisvc basic functionality")
{
    ...
    SECTION("GET_STATE") {
        ...
        aduc::Defer defer_rm_fifo([respFifoPath]() {
            unlink(respFifoPath);
        });
        ...
        aduc::Defer defer_close_req_fifo([reqFifo]() -> void {
            close(reqFifo);
        });

        // Write GET_STATE request
        int slen = strlen(respFifoPath);
        ApiWireRequestMsg req = { .ver = htons(1), .type = htons(ApiRequestType_GETSTATE), .len = htons((uint16_t)slen) };
        ...
        aduc::Defer defer_close_resp_fifo([respFifo]() -> void {
            close(respFifo);
        });
