...
TEST_CASE("API Service Container Tests", "[apisvc_container]")
{
    ...
    SECTION("GETSTATE with Container Timeouts")
    {
        ...
        std::cout << "Message send completed in " << send_elapsed << "ms" << std::endl;
    ...
    SECTION("Stress Test with Container Constraints")
    {
        ...
        for (int i = 0; i < num_iterations; i++)
        {
            ...
            int reqFifo = open(fifoPath, O_WRONLY);
            REQUIRE(reqFifo != -1);
            aduc::Defer defer_close_req([reqFifo]() -> void { close(reqFifo); });
            ...
            ApiWireRequestMsg req = { .ver = 1, .type = ApiRequestType_GETSTATE, .len = (uint16_t)slen };
            strncpy(req.data, respFifoPath, slen);
            REQUIRE(msg_send_req(reqFifo, &req) == 3 * sizeof(uint16_t) + slen);
