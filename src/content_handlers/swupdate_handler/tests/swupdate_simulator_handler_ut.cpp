...
TEST_CASE("Simulator Handler content handler prepare test")
{
    std::unique_ptr<ContentHandler> handler =
        SWUpdateSimulatorHandlerImpl::CreateContentHandler(workFolder, logFolder, filename);

    CHECK(handler);

    ADUC_PrepareInfo info = {};
    char updateType[21] = "microsoft/swupdate:1";
    char updateTypeName[19] = "microsoft/swupdate";
    ...
    SECTION("Prepare Fail with Wrong File Count")
    {
        ...
        CHECK(result.ExtendedResultCode == ADUC_ERC_SWUPDATE_HANDLER_PACKAGE_PREPARE_FAILURE_WRONG_FILECOUNT);
    }

    SECTION("SWUPDATE handler prepare wrong version")
    {
        info.fileCount = 1;
        info.updateTypeVersion = 2;
        ADUC_Result result = handler->Prepare(&info);
        CHECK(result.ResultCode == ADUC_PrepareResult_Failure);
        CHECK(result.ExtendedResultCode == ADUC_ERC_SWUPDATE_HANDLER_PACKAGE_PREPARE_FAILURE_WRONG_VERSION);
    }
}
