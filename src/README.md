# Device Update Agent — Source Module Catalog

Quick reference for developers working in the `src/` tree. Each section
describes one top-level directory: what it is, what it depends on, and where
to look first.

For the high-level architecture, component interactions, security model, and
...
Runs as a setuid binary to bridge the privilege gap between the unprivileged
agent process and operations that require root access. Only a well-defined set
of operations is supported. See the
[adu-shell README](adu-shell/README.md) for the full privilege model, process
boundary diagram, and trusted-user validation flow.
...
- [Architecture Overview](../docs/agent-reference/architecture-overview.md) —
  high-level design, security model, update lifecycle
- [adu-shell README](adu-shell/README.md) — privilege model and process
  boundary diagram
