...
<#
    .SYNOPSIS
        Create a sample update that resulted in a very large Update Manifest, and importing it to Device Update for IoT Hub
    ...
    .EXAMPLE
        ...
        PS >
        PS > ImportSampleMSOEUpdate-10.x.ps1 -AccountEndpoint sampleaccount.api.adu.microsoft.com -InstanceId sampleinstance `
...
# ------------------------------------------------------------
# Create the parent update that resulted in a very large Update Manifest
# ------------------------------------------------------------
...
$parentSteps = @()
$parentSteps += New-AduInstallationStep -Handler 'microsoft/apt:1' -Files $parentFile0 -HandlerProperties @{ 'installedCriteria'='apt-update-test-1.0'} -Description 'Install libcurl4-doc on host device'

$parentSteps += New-AduInstallationStep -Handler 'microsoft/apt:1' -Files $parentFile1 `
        `
        -HandlerProperties @{  `
            'installedCriteria'='apt-update-test-tree-1.0'; `
        ...
        `
        -Description 'Update with deteached Update Manifest file.'
...
# -----------------------------------------------------------
# Call ADU Import Update REST API to import the update above
# -----------------------------------------------------------
