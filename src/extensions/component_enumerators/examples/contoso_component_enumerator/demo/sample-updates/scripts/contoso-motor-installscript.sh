...
while [[ $1 != "" ]]; do
    ...
    #
    # Following arguments and parameters will be populated by the Scripts Handler.
    #
    --component-id)
        shift
        if [[ -z $1 || $1 == -* ]]; then
    ...
    if [ "$installed_criteria" == "" ]; then
        ...
        if [ $grep_ret -eq 0 ]; then
            ...
            if [ $copy_ret -ne 0 ]; then
                ...
                resultCode=0
                # ADUC_ERC_SCRIPT_HANDLER_CHILD_PROCESS_FAILURE_EXITCODE(exitCode) (0x30501000 + exitCode)
                extendedResultCode=$((0x30501000 + copy_ret))
