...
{
    ADUC_Retriable_Operation_Context* operationContext = RetriableOperationContextFromEnrollmentMqttLibCallbackUserObj(obj);
    ADUC_Enrollment_Request_Operation_Data* enrollmentData = EnrollmentDataFromRetriableOperationContext(operationContext);
    ADUC_MQTT_Common_HandlePublishAck(mosq, obj, props, reason_code, operationContext, enrollmentData->enrReqMessageContext.correlationId);
}
