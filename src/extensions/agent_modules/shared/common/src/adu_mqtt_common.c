    ...
    if (IsNullOrEmpty(messageContext->publishTopic))
    {
        ...
        if (isScoped)
        {
            messageContext->publishTopic =
                ADUC_StringFormat(PUBLISH_TOPIC_TEMPLATE_ADU_OTO_WITH_DU_INSTANCE, externalDeviceId, scopeId);
        }
        else
        {
    ...
    if (IsNullOrEmpty(messageContext->responseTopic))
    {
        ...
        if (isScoped)
        {
            messageContext->responseTopic =
                ADUC_StringFormat(SUBSCRIBE_TOPIC_TEMPLATE_ADU_OTO_WITH_DU_INSTANCE, externalDeviceId, scopeId);
        }
        else
        {
    ...
    const ADUC_Retriable_Operation_Context* context, ADUC_MQTT_Message_Context* messageContext, bool isScoped)
{
    ...
    ADU_MQTT_COMMUNICATION_MGR_STATE* commMgrState =
        CommunicationManagerStateFromModuleHandle((ADUC_AGENT_MODULE_HANDLE)context->commChannelHandle);
    ...
    if (mqtt_res != MOSQ_ERR_SUCCESS)
    {
        ...
        context->retryFunc(
            ...
            &context->retryParams[ADUC_RETRY_PARAMS_INDEX_CLIENT_TRANSIENT]);
    }
    ...
    const char* correlationId)
{
    ...
    case MQTT_RC_NOT_AUTHORIZED:
        ...
        Log_Warn(PubAckTraceFormat, reason_code, correlationId, mosquitto_reason_string(reason_code), "Retrying");
    ...
    case MQTT_RC_QUOTA_EXCEEDED:
        Log_Error(PubAckTraceFormat, reason_code, correlationId, mosquitto_reason_string(reason_code), "Canceling");
